...
const thirdLogin = {
  ...
  actions: {
    ...
    metamaskLogin({ commit }, params) {
      return new Promise((resolve, reject) => {
        metamaskLogin(params).then(response => {
          const loginData= response.data;
          resolve({ hasError: false, data: loginData })
        }).catch(error => {
          resolve({ hasError: true,  error });
        })
      })
    },
    metamaskVerify({ commit }, params) {
      return new Promise((resolve, reject) => {
    ...
    StoreBindingGoogleUserInfo({ commit }, params) {
      return new Promise((resolve, reject) => {
        const userId = params.userId;
        const { encryptPrivateKey } = params
